...
let title = '', servings = 1, steps = [], imageUrl = null, rawIngredients = []
if (recipeJson) {
...
? recipeJson.recipeInstructions.map((i) => (typeof i === 'string' ? i : (i?.text || ''))).filter(Boolean)
: []
...
const listCandidates = $('li').map((i, el) => $(el).text().trim()).get()
rawIngredients = listCandidates
.filter(t => /(\d|\bg\b|\bkg\b|\bml\b|\bl\b|cuill|oeuf|lait|farine|sucre)/i.test(t))
.slice(0, 40)
steps = $('p').map((i, el) => $(el).text().trim()).get().filter(Boolean).slice(0, 12)
...
// **Ne pas enregistrer directement** → la V1 demande de PRÉREMPLIR l’éditeur
res.json({
ok: true,
