...
  const aEndsOpen =
    /[,/&+–—-]\s*$/.test(a) || /\b(et|de|d['’]|du|des|à|a)\s*$/i.test(a);

  const bLooksContinuation =
    /^[A-ZÀ-ÖØ-Þa-zà-öø-ÿ]/.test(b) && !/^\d/.test(b) && b.length <= 60;
...
    if (
      low === 'diner' || low === 'dîner' ||
      low === 'preparation' || low === 'préparation' ||
      low === 'temps cuisson' || low === 'temps de cuisson' ||
      low === 'temps preparation' || low === 'temps de préparation' ||
      low === 'difficulté' || low === 'difficulte' ||
      low === 'portions' || low === 'directions'
    ) continue;
