...
// Helpers communs (identiques au generic pour cohérence)
...
module.exports = async function parseMarmiton($, url) {
  ...
  if (recipeJson) {
    ...
    // Fallback Marmiton : sélectionne les blocs typiques
    title =
      $('h1').first().text().trim() ||
      $('title').text().trim() ||
      'Recette importée';
    ...
    const pSteps = $('.recipe-preparation__list li, .preparation__list li, .recipe-steps li, p')
      .map((i, el) => $(el).text().trim())
      .get()
      .filter(Boolean);
    ...
    imageUrl =
      $('[itemprop="image"]').attr('src') || $('img').first().attr('src') || null;
