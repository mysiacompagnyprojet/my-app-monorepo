...
function NewRecipeInner() {
  ...
  // ✅ total = somme des costs affichés
  const totalCost = useMemo(() => {
    return ingredients.reduce((acc, i) => acc + (typeof i.costEur === 'number' ? i.costEur : 0), 0)
  }, [ingredients])

  // (optionnel) moyenne de prix d’achat unitaire (info)
  const avgUnitPriceBuy = useMemo(() => {
    const vals = ingredients
      .map((i) => (typeof i.unitPriceBuy === 'number' ? i.unitPriceBuy : null))
  ...
  async function recalcPrices() {
    try {
      ...
      // ✅ apiFetch renvoie déjà le JSON (ou throw si erreur)
      ...
      setIngredients((prev) => {
        ...
        for (let k = 0; k < enriched.length; k++) {
          ...
          copy[idx] = {
            ...
            // conserve l’affichage saisi par l’utilisateur (fraction, etc.)
  ...
  useEffect(() => {
    ...
    // petite file d'attente pour laisser le state/DOM se stabiliser
    setTimeout(() => {
      recalcPrices()
    }, 0)
      ...
      <section className="app-card p-6" style={{ marginTop: 16 }}>
        ...
        <div className="mt-4 grid gap-3">
          {ingredients.map((ing, idx) => (
            ...
            >
              ...
              >
                ...
                <div style={{ minWidth: 220, textAlign: 'right', fontSize: 13, opacity: 0.95 }}>
                  ...
                  <div className="app-muted" style={{ fontSize: 12 }}>
                    Achat (€/unité) :{' '}
                    {typeof ing.unitPriceBuy === 'number' ? ing.unitPriceBuy.toFixed(5) : '—'}
                  </div>
          ...
          <div style={{ marginTop: 8, textAlign: 'right' }}>
            ...
            <div className="app-muted" style={{ fontSize: 12, marginTop: 4 }}>
              Prix d’achat (€/unité) moyen :{' '}
              {typeof avgUnitPriceBuy === 'number' ? avgUnitPriceBuy.toFixed(5) : '—'}
            </div>
          </div>
        </div>
      </section>
